...
const create = (opts = defaultConfig) => {
    ...
    if (!opts.merchantLogin || !opts.password1 || !opts.password2) {
      throw new Error('Please provide merchantLogin, password1 and password2 to robokassa.js');
    }
    ...
    const generatePaymentUrl = (amount, description, invoiceID = 0) => {
      const hash = calcHash(`${options.merchantLogin}:${amount}:${invoiceID}:${options.password1}`);
      const args = {
     ...
     */
    const getAmountPlusCharges = (wantedAmount, currencyLabel = null) => api.get('/GetRates', {
     ...
     * @param {string} currencyLabel Currency code for which amount should be calculated 
     */
    const getAmountMinusCharges = (payableAmount, currencyLabel = null) => api.get('/CalcOutSumm', {
      IncCurrLabel: currencyLabel,
